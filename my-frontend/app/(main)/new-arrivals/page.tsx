...
export default function NewArrivalsPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true);
        const data = await productAPI.getNewArrivals(24);
        setProducts(data);
        setError(null);
      } catch (err) {
          ...
          {loading ? (
            ...
            <>
              ...
              {sortedProducts.length === 0 ? (
                <div className="rounded-2xl border border-dashed border-brand-primary-light bg-white py-16 text-center shadow-sm">
                  <p className="text-brand-muted font-medium">{t("label.noProductsFound")}</p>
                </div>
              ) : (
